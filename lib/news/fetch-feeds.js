...
function stripHtml(html) {
  return html
    ...
    .replace(/&#8221;/g, '"')
    .replace(/&#8211;/g, '–')
    // Em dashes render as plain hyphens: the site's own copy carries none
    // (2026-08-04, Hamza's call), and external headlines shouldn't reintroduce
    // them. The \u2014 escape stands in for the literal character, so this
    // file itself contains no em dash and a future strip-them-all sweep (like
    // the one that turned this line into a no-op /-/g) cannot break the regex.
    .replace(/&#8212;/g, '-')
    .replace(/\u2014/g, '-')
    .replace(/\s+/g, ' ')
    .trim();
