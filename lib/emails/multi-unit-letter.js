...
<tr><td>
  ...
  exactly <strong>${n(count)}</strong> on the market in all of Mississauga - ${esc(breakdown)}.
  ...
  Multi-unit moves slowly here - the buyer pool is small, which is precisely what makes it
  ...
  large share of it toward your income when sizing the mortgage - how much varies by lender,
  ...
  TRREB doesn't publish a multi-unit category - no average price, no sale-to-list benchmark.
  ...
  I'll send you all ${n(count)} with real numbers run on each - what the units could actually
  ...
  <p style="${P}">Or see all ${n(count)} on the site right now:
  <a href="${u('/listings?type=Duplex%2FMulti')}" style="color:${LINKBLUE};text-decoration:underline;">the multi-unit filter, already applied</a>.
  Give the page a moment to finish scanning - these are the oldest listings on the
  market, so they load last.</p>
  ...
  <div style="border-top:1px solid #E5E7EB;margin-top:26px;padding-top:14px;">
    <p style="margin:0 0 10px;font-family:${SERIFISH};font-size:11px;line-height:1.65;color:#9CA3AF;">
      ...
      For information only - not mortgage, appraisal or investment advice. Listing data
...
export function buildMultiUnitText({ email, name, data } = {}) {
  ...
  return [
    ...
    '',
    `Or see all ${n(count)} with the filter already applied:`,
    u('/listings?type=Duplex%2FMulti'),
    '(Give the page a moment - these are the oldest listings on the market, so',
    'they load last.)',
    '',
    '- Hamza',
