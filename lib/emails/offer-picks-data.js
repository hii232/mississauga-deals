 ...
 *
 * Freehold and condo townhouses behaved very differently last month (freehold
 * sold at 101% of ask, condo townhouses at 98%), so the anchor genuinely
 * matters. `mapType` now distinguishes them outright — "Condo Townhouse" is a
 * canonical type of its own — so this reads the type directly instead of
 * inferring from whether a condo fee happens to be present.
 *
 * The fee-based inference is KEPT as a fallback for any row that still arrives
 * typed as a plain "Townhouse" (a cached payload from before the split, or a
 * subtype spelling the mapper has not seen). Absent a fee it resolves to
 * FREEHOLD, the higher of the two ratios, which produces a HIGHER offer and so
 * UNDERSTATES the buyer's leverage — wrong in that direction costs a few
 * thousand of negotiating room; wrong the other way tells someone to lowball a
 * property that sells over ask and loses them the deal.
 */
export function resolveTypeKey(listing) {
  if (listing?.type === 'Condo Townhouse') return 'condoTown';
