...
import { cityAliases, citySubarea, listingInSubarea } from '@/lib/constants';
...
export function poolForSearch(filters, mississaugaListings, gtaListings) {
  ...
  // An amalgamated district (Etobicoke, Scarborough, Stoney Creek, …) is never
  // the `city` on a row — the board doesn't use those names, so those rows
  // arrive under Toronto/Hamilton and an exact-name compare would match
  // nothing, silently killing the alert of anyone who saves a search on one of
  // those eight pages. Same predicate the feed request uses (CITY_SUBAREAS).
  const subarea = citySubarea(city);
  if (subarea) return gtaListings.filter((l) => listingInSubarea(l, subarea));
  // Match on every name the municipality can appear under, not just the one
  // the search was saved with. Only Halton Hills/Georgetown has two
  // (CITY_ALIAS_GROUPS in lib/constants.js) — and a search saved from
  // /gta/georgetown would otherwise match nothing at all the moment the feed
  // labels those rows 'Halton Hills', silently killing that subscriber's alert.
  const names = new Set(cityAliases(city));
  return gtaListings.filter((l) => names.has(String(l.city || '').trim()));
}
