...
/**
 * Detect number of units from property type and listing remarks.
 * Returns: { units: number, type: 'duplex'|'triplex'|'fourplex'|'multiplex'|null }
 */
export function detectMultiUnit(propType, remarks) {
  const type = (propType || '').toLowerCase();
  const text = (remarks || '').toLowerCase();

  // From property subtype (most reliable)
  ...
  // From remarks — look for explicit unit counts
  const unitCountMatch = text.match(/(\d+)\s*(?:unit|apt|apartment|suite|rental)/i);
  if (unitCountMatch) {
    const n = parseInt(unitCountMatch[1]);
    if (n >= 2 && n <= 10) {
      const t = n === 2 ? 'duplex' : n === 3 ? 'triplex' : n === 4 ? 'fourplex' : 'multiplex';
      return { units: n, type: t };
    }
  }

  // From remarks keywords
  if (/\b(fourplex|four-plex|quadruplex|4-plex|4 plex)\b/.test(text)) return { units: 4, type: 'fourplex' };
  if (/\b(triplex|tri-plex|3-plex|3 plex)\b/.test(text)) return { units: 3, type: 'triplex' };
  if (/\b(duplex|du-plex)\b/.test(text)) return { units: 2, type: 'duplex' };
  if (/\b(upper\s*(and|&)\s*lower|two\s*(?:self[- ]contained|separate)\s*unit|2\s*(?:self[- ]contained|separate)\s*unit)\b/.test(text)) return { units: 2, type: 'duplex' };
