import { LRT_CORRIDOR_HOODS, getCityRentTier } from './constants';
 ...
 *
 * City tier now comes from the single canonical getCityRentTier() (constants.js)
 * instead of a second, hand-typed table - this file used to carry its own
 * MULTI_UNIT_CITY_TIER, and it had drifted from the canonical one for every
 * single overlapping city (e.g. Toronto 1.05 here vs 1.15 canonical, Brampton
 * 0.85 vs 0.90), always LOWER, so every GTA multi-unit listing outside
 * Mississauga - exactly the legal-duplex/triplex/multiplex properties this
 * site is built to surface as high-cash-flow plays - had its rent, and
 * therefore its cash flow and cap rate, systematically UNDERSTATED. Confirmed
 * this is the live path: process-listings.js calls this for every detected
 * multi-unit listing, Mississauga and GTA alike.
 *
 * Returns { totalRent, units: [{ beds, rent }] }
 */
export function estimateMultiUnitRent(unitCount, totalBeds, propType, city) {
  const tier = getCityRentTier(city);
