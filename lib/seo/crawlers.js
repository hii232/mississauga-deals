...
export function classifyCrawler(userAgent) {
  ...
  if (generic) {
    // Unrecognised bot: label it with its NAME, not the UA's head. The first
    // production night proved the head-slice useless for exactly the bot that
    // mattered - the top crawler logged as
    //   other:Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatibl
    // 60 characters of pure boilerplate, truncated one word before the part
    // that identifies anything. Bot UAs bury their identity mid-string, almost
    // always after a "compatible;" marker or in the token that contains the
    // bot-word itself - so extract that, and fall back to the head slice only
    // when neither exists.
    // compat is only trusted when its capture is itself bot-ish: legacy IE
    // spells "compatible; MSIE 9.0" and would put browser boilerplate in the
    // label. The token fallback strips trailing ;,) so "MysteryBot/1.0;" in
    // mid-UA position comes out clean.
    const compat = ua.match(/compatible;?\s*([^;)]{2,60})/i)?.[1]?.trim();
    const token = ua.match(/(\S*(?:bot|crawler|spider|scraper|curl|wget|python-requests|headless|scrapy)\S*)/i)?.[1]?.replace(/[;,)]+$/, '');
    const label = (compat && GENERIC.test(compat) ? compat : null) || token || compat || ua.slice(0, 60);
    return { name: 'other:' + label.slice(0, 60), disallowed: false };
  }
  return null;
}
