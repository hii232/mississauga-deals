...
console.log('\nUnknown bots surface their NAME so the list can grow');
...
check('unknown bot label names the bot, not the boilerplate',
  unknown?.name === 'other:SomeNewBot/3.0', JSON.stringify(unknown));
check('unknown bot label is bounded', unknown.name.length <= 66, String(unknown.name.length));
check('curl label keeps its version', classifyCrawler('curl/8.4.0')?.name === 'other:curl/8.4.0',
  JSON.stringify(classifyCrawler('curl/8.4.0')));
check('python-requests named', classifyCrawler('python-requests/2.31.0')?.name === 'other:python-requests/2.31.0');

console.log('\nThe first production night: the label must survive real UA shapes');
// The top crawler on 2026-08-04 logged as
// "other:Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatibl" -
// sixty characters of boilerplate, cut one word before the identity. This
// shape (no platform paren, name after "compatible;") must now yield the name.
const prodShape = classifyCrawler(
  'Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatible; MysteryBot/2.1; +https://example.com/bot)'
);
check('OpenAI-style UA names the bot after compatible;',
  prodShape?.name === 'other:MysteryBot/2.1', JSON.stringify(prodShape));
// Legacy IE puts BROWSER boilerplate after "compatible;" - the label must not
// trust that capture unless it is itself bot-ish, and must strip trailing
// punctuation from the mid-UA token it falls back to.
const ieShape = classifyCrawler('Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; LegacyCrawler/1.0)');
check('compatible; MSIE does not become the label',
  ieShape?.name === 'other:LegacyCrawler/1.0', JSON.stringify(ieShape));
const headless = classifyCrawler('Mozilla/5.0 (X11; Linux x86_64) HeadlessChrome/126.0.0.0 Safari/537.36');
check('HeadlessChrome named', headless?.name === 'other:HeadlessChrome/126.0.0.0', JSON.stringify(headless));
