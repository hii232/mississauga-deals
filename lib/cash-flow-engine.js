 ...
 * Properties inside the (amalgamated) City of Toronto pay a SECOND land-transfer
 * tax on top of the provincial one. For residential purchases the base MLTT
 * brackets mirror the provincial brackets exactly (0.5% → 2.5% at the same
 * thresholds), so a Toronto buyer pays ~double the land-transfer tax of the same-
 * priced purchase elsewhere in the GTA. (Toronto's 2024+ luxury surtax above $3M
 * is omitted — negligible for typical investment inventory.)
 */
export function calculateTorontoMLTT(purchasePrice) {
  return calculateOntarioLTT(purchasePrice);
}

// Amalgamated City of Toronto districts subject to the MLTT.
const TORONTO_MLTT_CITIES = new Set([
  'toronto', 'north york', 'scarborough', 'etobicoke', 'east york', 'york',
]);

export function hasTorontoMLTT(city) {
  return TORONTO_MLTT_CITIES.has(String(city || '').trim().toLowerCase());
}

// Total land-transfer tax for a purchase: provincial LTT everywhere, plus the
// Toronto MLTT when the property is in Toronto.
export function totalLandTransferTax(price, city = null) {
  return calculateOntarioLTT(price) + (hasTorontoMLTT(city) ? calculateTorontoMLTT(price) : 0);
}

/**
 * Calculate Cash-on-Cash Return with proper closing costs.
 * `city` lets Toronto listings include the municipal LTT (double land-transfer
 * tax) in cash invested — omitting it overstated CoC on every Toronto listing.
 */
export function calculateCashOnCash(annualCashFlow, price, downPct = 20, city = null) {
  const downPayment = price * (downPct / 100);
  const ltt = totalLandTransferTax(price, city);
  const closingCosts = ltt + 3000; // LTT (incl. Toronto MLTT) + legal/title/misc
...
/**
 * Get full closing costs breakdown. Includes the Toronto municipal LTT as a
 * separate line when the property is in Toronto.
 */
export function getClosingCosts(price, downPct = 20, city = null) {
  const downPayment = price * (downPct / 100);
  const ltt = calculateOntarioLTT(price);
  const mltt = hasTorontoMLTT(city) ? calculateTorontoMLTT(price) : 0;
  const legalAndTitle = 2500;
  const inspectionMisc = 500;
  const closingCosts = ltt + mltt + legalAndTitle + inspectionMisc;
