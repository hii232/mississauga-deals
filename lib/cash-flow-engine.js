...
export function calcMonthly(price, downPct = 20, rate = 4.89, years = 25) {
  ...
  if (rate === 0) return Math.rhound(principal / (years * 12));
...
export function calculateCashFlow(price, rent, {
  ...
  // Maintenance: greater of % of rent OR 1% of property value annually
    const maintenanceByRent = Math.round(rent * (maintenancePct / 100));
    const maintenanceByValue = Math.round(price * 0.01 / 12);
    const maintenance = Math.max(maintenanceByRent, maintenanceByValue);
