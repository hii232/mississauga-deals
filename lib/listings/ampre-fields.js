...
// Keyed by token FINGERPRINT, not just time. The whole point of this probe is
// that field permissions follow the token, so a cache that ignores which token
// produced it answers the wrong question the moment the token changes — e.g.
// swapping an IDX token for a VOW-scoped one, which is exactly the upgrade
// this module was built to pick up "with no code change and no deploy". A warm
// instance would otherwise keep serving the IDX-derived field list, and its
// DaysOnMarket/Latitude/Longitude nulls, for up to an hour after the switch.
// Only a short prefix+length is stored: enough to detect a change, never
// enough to reconstruct the secret if this object is ever dumped in a log.
let probeCache = { at: 0, fields: null, clean: false, tokenFp: null };

const tokenFingerprint = (t) => (t ? `${String(t).slice(0, 8)}:${String(t).length}` : 'none');
...
export async function probeSupportedFields(token, { force = false } = {}) {
  ...
  // A different token invalidates immediately: its permissions are a different
  // question, so the cached answer is not stale, it is about something else.
  if (!force && probeCache.fields && probeCache.tokenFp === fp && Date.now() - probeCache.at < ttl) {
  ...
  if (errored && probeCache.fields) {
    ...
    probeCache = { at: Date.now(), fields: merged, clean: false, tokenFp: fp };
    return merged;
  }
  probeCache = { at: Date.now(), fields, clean: !errored, tokenFp: fp };
  return fields;
}
 ...
 *
 * `media: false` drops the two photo-bearing tiers. It exists for callers that
 * AGGREGATE the feed rather than render it: the Media $expand returns roughly
 * five CDN size-variants per photo, so it is the dominant cost of a feed
 * request (~18k media rows on a 100-row page) and an aggregate never reads a
 * single one of them. The remaining tiers are the SAME queries the cascade
 * already falls through to today — no new $select is introduced — so the rows
 * are byte-identical apart from the photo arrays.
 */
export async function fetchWithFieldTiers(baseUrl, tail, token, { media = true } = {}) {
  ...
  const tiers = (media
    ...
    : [
        { name: 'probed-nomedia', url: q(select, null) },
        { name: 'core-nomedia', url: q(SELECT_CORE, null) },
      ]);
