...
/**
 * Request header meaning "serve the cached aggregate; do NOT start a walk".
 *
 * Lives here rather than in the route so a page can send it without importing
 * a route handler - that import would pull buildGtaScreenerAggregates and the
 * whole feed-walk module graph into the page bundle.
 *
 * A header, not a query param, on purpose: the CDN cache key must stay exactly
 * `/api/gta-screener` so a read-only caller can be served the cron's real body.
 * A `?cached=1` variant would be its own cache entry and never see it.
 */
export const READ_ONLY_HEADER = 'x-screener-read-only';
...
export async function buildGtaScreenerAggregates(origin, {
  ...
  // Publish when the walk ATTEMPTED every page and the fill bar is met.
  // The old rule also required zero dropped pages (walkedAll), which vetoed
  // real data: live capture 2026-08-08 - 234/234 pages walked, fillRatio
  // 0.996, TWO pages dropped by upstream blips -> nothing published, and
  // every /gta city page sat on "analyzing..." skeletons indefinitely. The
  // withhold rule exists to refuse TRUNCATED walks (a budget stop leaves a
  // recency-biased prefix, which no fill ratio can launder - stoppedEarly
  // still refuses those unconditionally). A fully-attempted walk missing a
  // couple of scattered pages is a different thing: the >= MIN_FILL (95%)
  // bar was ALREADY accepting up to 5% missing rows as aggregate-safe, so
  // letting sub-5% residual drops - now rare, after fetch-feed's retry pass
  // - veto the publish contradicted the bar this module itself set. Do not
  // lower MIN_FILL; that rule stands.
  const attemptedAll = !walk.stoppedEarly && (walk.requested || 0) >= (walk.totalPages || 1);
  const complete = attemptedAll && fillRatio >= MIN_FILL;
