...
/**
 * TRUE lower bound on days-on-market — never a fabrication.
 * A listing was on market BEFORE its last modification, and before we first
 * saw it, so max(feed DOM, days-since-last-change, days-since-first-seen) is
 * provably <= the real figure. Displayed as "N+ days"; 0 = nothing known.
 */
export function computeDomFloor(dom, daysSinceUpdate, daysSinceFirstSeen) {
  return Math.max(
    Number(dom) > 0 ? Number(dom) : 0,
    Number.isFinite(daysSinceUpdate) ? daysSinceUpdate : 0,
