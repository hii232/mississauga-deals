...
export function computeScreenerMetrics(listings) {
  ...
  return {
    ...
    // The IDs behind cfPlusCount, best first. Cash-flow-positive listings are
    // ~6 in ~2,450, and the /listings Cash Flowing filter used to make every
    // phone re-walk the whole feed to find them; the server walk already knows
    // exactly which ones they are, so publish them and let the client fetch
    // those listings directly. Capped to bound the payload. DELIBERATELY NOT
    // in EMPTY_SCREENER_METRICS: normalizeSummary requires every key of that
    // template to be a finite Number, so adding an array key there would make
    // it reject EVERY summary and put the Deal Screener back on skeletons
    // site-wide. As an extra field it rides the same JSON but is invisible to
    // the summary validator, which copies only SUMMARY_KEYS.
    cfPlusIds: cfPlusListings
      .slice()
