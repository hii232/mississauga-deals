...
export function processListings(rawListings) {
  ...
  const processed = filtered.map((l, i) => {
    ...
    // CRITICAL: Always use mainBeds for base rent, never the API's pre-calculated estimatedRent
    // which uses total beds and would double-count when we add basement income below
    ...
    if (hoodRents) {
      ...
      // For non-suite: use API estimate or price ratio as fallback
      if (hasSuitePotential) {
        ...
        if (/duplex|triplex|fourplex|multiplex/i.test(propType)) fbAdj += 800;
