...
export function processListings(rawListings) {
  ...
  return filtered.map((l, i) => {
    ...
    const beds = l.beds || 0;
    const baseRent = l.estimatedRent || l.rent || Math.round(price * 0.0042);
    ...
    // Tiered basement detection
    const basementTier = detectBasementTier(remarks) || (l.hasSuite ? 'legal' : null);
