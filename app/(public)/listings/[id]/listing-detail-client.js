...
export default function PropertyDetailClient({ initialListing = null }) {
  const params = useParams();
  // `initialListing` is fetched and processed on the SERVER by page.js, so this
  // component's markup renders into the HTML with real content. It used to
  // start null and fill in from a useEffect, which meant Googlebot (and any
  // visitor before hydration) received an empty shell on all ~2,500 property
  // pages. The fetch below is kept as the fallback for when the server call
  // fails, so behaviour degrades rather than breaks.
  ...
  useEffect(() => {
    if (initialListing) return; // server already provided the data
